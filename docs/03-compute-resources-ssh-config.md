...
> [!NOTE]
> This alternative guide goes through the process of setting up SSH congurations assuming that the nodes and servers are on different hosts. We will use the hostname instead of IP addresses.
...
```
Host server
  HostName 172.17.0.3 # IP Address of server
  User root
  Port 22

Host node-0
  HostName 172.17.0.4 # IP Address of node-0
  User root
  Port 22

Host node-1
  HostName 172.17.0.5 # IP Address of node-1
  User root
  Port 22
```
